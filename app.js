...
if (!isDashboard) {
    ...
    if (tabBtns.length && loginFormDiv && signupFormDiv) {
        tabBtns.forEach(btn => {
    ...
    if (signupBtn) {
        signupBtn.addEventListener("click", async () => {
            if (signupPassword.value.length < 6) {
                signupError.innerText = "Nenosiri lazima iwe angalau herufi 6.";
                return;
            }
            try {
...
// --------------------------------------------------------------
// DASHBOARD PAGE (hapa ndipo kadi zinaundwa)
// --------------------------------------------------------------
if (isDashboard) {
    ...
    onAuthStateChanged(auth, async (user) => {
        ...
        // Kila kitufe kinawasha WhatsApp na kuhifadhi ombi
